...
- **Cursor's included usage, if you use Cursor.** Settings → Data Source → **Monitor Cursor
  ...
  included usage is left this billing cycle, on models picked by name — the one that runs out
  first, so it leads — and through Auto, the same two bars Cursor's own dashboard draws. Cursor offers individual plans no usage API, so Tokenmax asks the
